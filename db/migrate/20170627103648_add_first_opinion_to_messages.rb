class AddFirstOpinionToMessages < ActiveRecord::Migration
  def up
    execute "ALTER TABLE messages DROP CONSTRAINT IF EXISTS validate_attributes_team_id"
    execute "ALTER TABLE messages DROP CONSTRAINT IF EXISTS  validate_attributes_channel_user"
    ...
    "ALTER TABLE messages ADD CONSTRAINT validate_attributes_channel_user
      CHECK (
        ...
        OR
        (
          (provider = 'slack' OR provider = 'facebook' OR provider = 'kik' OR provider = 'first_opinion')
